...
namespace Snaptrude
{
    public class FloorTypeStore : TypeStore<ST_Layer[], FloorType>
    {
        public override string KeyAdapter(ST_Layer[] floorLayers, FloorType _defaultType)
        {
            string layers = floorLayers[0].BaseTypeName;
            foreach(ST_Layer w in floorLayers)
            {
                layers += "_" + w.ToString();
        ...
        public override void TypeModifier(ST_Layer[] floorLayers, FloorType floorType)
        {
            ...
            bool sawCore = false;
            int coreIndex = 0;
            foreach (ST_Layer floorLayer in floorLayers)
            {
