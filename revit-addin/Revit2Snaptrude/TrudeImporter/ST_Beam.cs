...
namespace Snaptrude
{
    public class ST_Beam : ST_Abstract
    {
        ...
        public static ST_Beam FromMassData(JToken massData)
        {
            ST_Beam st_beam = new ST_Beam();

            st_beam.Name                = STDataConverter.GetName(massData);
            st_beam.Position            = STDataConverter.GetPosition(massData);
            st_beam.CenterPosition      = STDataConverter.GetCenterPosition(massData);
            st_beam.levelNumber         = STDataConverter.GetLevelNumber(massData);
            st_beam.vertices            = STDataConverter.GetVertices(massData, 6);
            st_beam.levelId             = TrudeImporter.LevelIdByNumber[st_beam.levelNumber];
        ...
        public void CreateBeam(Document doc)
        {
            ...
            familyName = shapeProperties is null ? $"beam_custom_{TrudeImporter.RandomString(5)}" : $"beam_{shapeProperties.ToFamilyName()}";
